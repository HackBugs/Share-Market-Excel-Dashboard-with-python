...
if predictive_mode and indicator == "GDP" and not df.empty:
    ...
    df_pred = df_pred.melt(id_vars=["country", "iso3", "military_personnel", "tanks", "aircraft", "naval_vessels", "missiles", "conflict", "intensity", "sentiment"], value_vars=["gdp_value", "gdp_2023"], var_name="year", value_name="gdp_value")
...
def create_visualization():
    ...
    current_indicator_col = "gdp_value" if predictive_mode and indicator == "GDP" else indicator_col
...
st.markdown(f"""
    <div style='display: flex; align-items: center; animation: glow 2s infinite alternate;'>
        <img src='https://images.weserv.nl/?url=https://raw.githubusercontent.com/HackBugs/Website-Collection/main/Website_Data/earth1.png&w=60&h=60&fit=cover&q=80
' style='margin-right: 1rem; width: 60px; filter: drop-shadow(0 0 10px #4ecca3);' alt='Logo'>
...
if not df.empty:
    ...
    if show_military:
        display_cols.extend(["military_personnel", "tanks", "military", "aircraft", "naval_vessels", "missiles"])
    if highlight_conflicts:
