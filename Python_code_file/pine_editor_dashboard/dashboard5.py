...
def validate_pine_script(pine_script):
    ...
    # Expanded supported functions for Alpha Predator script
    supported_functions = ['sma', 'ema', 'rsi', 'macd', 'bbands', 'vwma', 'crossover', 'highest', 'lowest', 'plotshape', 'line.new']
    if 'ta.' in pine_script and not any(f'ta.{func}' in pine_script for func in supported_functions):
        errors.append("Unsupported ta.* function detected")
    
    # Silently ignore unsupported features (no warnings)
    # if 'alertcondition(' in pine_script:
    #     st.warning("alertcondition is not supported in this dashboard.")
    # if 'table.new(' in pine_script:
    #     st.warning("table.new is not supported in this dashboard.")
...
def create_candlestick_chart(data, symbol, company_name, show_instructions, pine_script=None, mtf_data=None, show_mtf=False, 
    ...
    if pine_script:
        try:
            ...
            # Support for Alpha Predator script
            if 'Alpha Predator' in pine_script:
                # VWMA (already handled above)
                
                # Liquidation Zones
                lookback = 50
                if 'liquidationLookback = input(' in pine_script:
                    start = pine_script.find('liquidationLookback = input(') + len('liquidationLookback = input(')
                    end = pine_script.find(',', start)
                    try:
                        lookback = int(pine_script[start:end])
                    except ValueError:
                        st.error("Invalid liquidation lookback parameter")
                        return fig
                
                data['High_Liq'] = data['High'].rolling(window=lookback).max()
                data['Low_Liq'] = data['Low'].rolling(window=lookback).min()
                data['Mid_Liq'] = (data['High_Liq'] + data['Low_Liq']) / 2
                
                fig.add_trace(
                    go.Scatter(
                        x=data.index,
                        y=data['High_Liq'],
                        name="High Liquidation",
                ...
                if 'thresholdVolumeSpike = input(' in pine_script:
                    start = pine_script.find('thresholdVolumeSpike = input(') + len('thresholdVolumeSpike = input(')
                    end = pine_script.find(',', start)
                    try:
                        volume_threshold = float(pine_script[start:end])
                    except ValueError:
                        st.error("Invalid volume spike threshold")
                        return fig
                
                data['Avg_Volume'] = data['Volume'].rolling(window=20).mean()
                data['Volume_Spike'] = data['Volume'] > data['Avg_Volume'] * volume_threshold
                
                # Momentum
                data['Momentum'] = data['Close'] - data['VWMA']
                data['Momentum_Up'] = data['Momentum'] > 0
                data['Momentum_Down'] = data['Momentum'] < 0
                
                # RSI (already handled above)
                data['Overbought'] = data['RSI'] >= 70
                data['Oversold'] = data['RSI'] <= 30
                
                # Breakout Signals
                data['Breakout_Long'] = (data['Close'] > data['High_Liq']) & data['Volume_Spike'] & data['Momentum_Up']
                data['Breakout_Short'] = (data['Close'] < data['Low_Liq']) & data['Volume_Spike'] & data['Momentum_Down']
                
                # Reversal Signals
                data['Reversal_Long'] = (data['Close'] >= data['Low_Liq']) & (data['Close'] <= data['Mid_Liq']) & data['Oversold'] & data['Volume_Spike']
                data['Reversal_Short'] = (data['Close'] <= data['High_Liq']) & (data['Close'] >= data['Mid_Liq']) & data['Overbought'] & data['Volume_Spike']
                
                # Plot Signals
                breakout_long_idx = data[data['Breakout_Long']].index
                breakout_short_idx = data[data['Breakout_Short']].index
                reversal_long_idx = data[data['Reversal_Long']].index
                reversal_short_idx = data[data['Reversal_Short']].index
                
                if not breakout_long_idx.empty:
                    fig.add_trace(
                        go.Scatter(
                            x=breakout_long_idx,
                            y=data.loc[breakout_long_idx]['Close'] * 0.995,
                            mode='markers+text',
                            name='Breakout Long',
                            marker=dict(symbol='label-up', size=10, color='green'),
                            text=['BUY'] * len(breakout_long_idx),
                ...
                if not breakout_short_idx.empty:
                    fig.add_trace(
                        go.Scatter(
                            x=breakout_short_idx,
                            y=data.loc[breakout_short_idx]['Close'] * 1.005,
                            mode='markers+text',
                            name='Breakout Short',
                            marker=dict(symbol='label-down', size=10, color='red'),
                            text=['SELL'] * len(breakout_short_idx),
                ...
                if not reversal_long_idx.empty:
                    fig.add_trace(
                        go.Scatter(
                            x=reversal_long_idx,
                            y=data.loc[reversal_long_idx]['Close'] * 0.995,
                            mode='markers',
                            name='Reversal Long',
                ...
                if not reversal_short_idx.empty:
                    fig.add_trace(
                        go.Scatter(
                            x=reversal_short_idx,
                            y=data.loc[reversal_short_idx]['Close'] * 1.005,
                            mode='markers',
                            name='Reversal Short',
...
def main():
    ...
    if filtered_companies:
        ...
        # Pine Script Editor (from indi.py)
        st.sidebar.markdown("---")
        ...
        with tabs[1]:
            if st.session_state.show_pine_editor:
                st.subheader("Pine Script Editor")
                
                st.markdown("""
                **Pine Script Help:**
                - Write your custom indicators in Pine Script
                - Click "Apply Script" to add them to the chart
                - Supported indicators: VWMA, RSI, MACD, Bollinger Bands, Crossover, Alpha Predator
                - Unsupported: Alerts, Info Tables (silently ignored)
                """)
