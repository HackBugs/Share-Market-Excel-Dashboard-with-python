...
def fetch_current_price(ticker: str) -> Tuple[float, str]:
    try:
        stock = yf.Ticker(ticker)
        # Fetch 1-minute data for the last day to get the most recent price
        ...
        logger.debug(f"Current price for {ticker}: {current_price} at {timestamp}")
...
# Function to calculate Fibonacci levels based on recent data
def calculate_fibonacci_levels(df: pd.DataFrame) -> dict:
    if df.empty:
        return {}
    # Use last 5 days for Fibonacci to focus on recent price action
    recent_df = df.tail(5)
...
# Function to calculate pivot points based on last day's data
...
# Function to calculate VWAP based on recent data
def calculate_vwap(df: pd.DataFrame) -> pd.Series:
    if df.empty:
        return pd.Series()
    # Use last day for VWAP to focus on current price action
    recent_df = df.tail(1)
    typical_price = (recent_df['High'] + recent_df['Low'] + recent_df['Close']) / 3
    return (typical_price * recent_df['Volume']).cumsum() / recent_df['Volume'].cumsum()
...
# Function to generate recommendation based on current price
def generate_recommendation(df: pd.DataFrame, indicators: pd.DataFrame, current_price: float) -> tuple:
    ...
    try:
        ...
        # MACD
        if indicators['MACD'][-1] > indicators['Signal'][-1]:
            score += 2 * weights['MACD']
            signals.append("MACD Bullish (Buy)")
            contributions['MACD'] = {'signal': 'Bullish (Buy)', 'score': 2 * weights['MACD'], 'weight': weights['MACD']}
        elif indicators['MACD'][-1] < indicators['Signal'][-1]:
            score -= 2 * weights['MACD']
            signals.append("MACD Bearish (Sell)")
            contributions['MACD'] = {'signal': 'Bearish (Sell)', 'score': -2 * weights['MACD'], 'weight': weights['MACD']}
        else:
            contributions['MACD'] = {'signal': 'Neutral', 'score': 0, 'weight': weights['MACD']}
        
        # SMA
        if current_price > indicators['SMA1'][-1] > indicators['SMA2'][-1]:
            score += 2 * weights['SMA']
            signals.append("Price Above SMA (Buy)")
            contributions['SMA'] = {'signal': 'Above SMA (Buy)', 'score': 2 * weights['SMA'], 'weight': weights['SMA']}
        elif current_price < indicators['SMA1'][-1] < indicators['SMA2'][-1]:
            score -= 2 * weights['SMA']
            signals.append("Price Below SMA (Sell)")
            contributions['SMA'] = {'signal': 'Below SMA (Sell)', 'score': -2 * weights['SMA'], 'weight': weights['SMA']}
        else:
        ...
        # Ichimoku
        if (current_price > indicators['Senkou_Span_A'][-1] and 
            current_price > indicators['Senkou_Span_B'][-1] and
        ...
        elif (current_price < indicators['Senkou_Span_A'][-1] and 
              current_price < indicators['Senkou_Span_B'][-1]):
            score -= 2 * weights['Ichimoku']
        ...
        # VWAP
        if current_price > indicators['VWAP'][-1]:
        ...
        elif current_price < indicators['VWAP'][-1]:
            score -= 1 * weights['VWAP']
        ...
        # PSAR
        if current_price > indicators['PSAR'][-1]:
            score += 1 * weights['PSAR']
            signals.append("Price Above PSAR (Buy)")
            contributions['PSAR'] = {'signal': 'Above PSAR (Buy)', 'score': 1 * weights['PSAR'], 'weight': weights['PSAR']}
        elif current_price < indicators['PSAR'][-1]:
            score -= 1 * weights['PSAR']
            signals.append("Price Below PSAR (Sell)")
            contributions['PSAR'] = {'signal': 'Below PSAR (Sell)', 'score': -1 * weights['PSAR'], 'weight': weights['PSAR']}
        else:
...
def generate_report(ticker: str, info: dict, df: pd.DataFrame, indicators: pd.DataFrame, recommendation: str, signals: list, score: float, fib_levels: dict, pivot: float, support1: float, resistance1: float, backtest_results: tuple, currency_symbol: str, position_size: float, take_profit: float, sentiment: str, current_price: float, price_timestamp: str) -> str:
    if df.empty:
    ...
    ichimoku_status = "Above" if not indicators.empty and pd.notnull(current_price) and \
        pd.notnull(indicators['Senkou_Span_A'][-1]) and pd.notnull(indicators['Senkou_Span_B'][-1]) and \
        current_price > max(indicators['Senkou_Span_A'][-1], indicators['Senkou_Span_B'][-1]) else "Below"
    ...
    stop_loss = format_value((current_price - 2 * indicators['ATR'][-1]) if not indicators.empty and pd.notnull(indicators['ATR'][-1]) else None)
...
- **Current Price**: {currency_symbol}{format_value(current_price)} (Source: yfinance, Timestamp: {price_timestamp})
...
## Conclusion
This analysis suggests a **{recommendation}** position based on advanced technical indicators and sentiment analysis, using the current price from yfinance. Use the risk management parameters and backtest results to inform your trading decisions. Always combine with fundamental analysis.

## Price Source
The current price of {currency_symbol}{format_value(current_price)} was retrieved from **yfinance** at {price_timestamp}.
"""
    return report
...
The **{recommendation}** recommendation is derived from a multi-indicator system with weighted scoring based on the current price:

#### Indicators Used
- **RSI**: Contributes {'RSI Oversold (Buy)' in signals and '2.0' or 'RSI Overbought (Sell)' in signals and '-2.0' or '0'}.
- **MACD**: Contributes {'MACD Bullish (Buy)' in signals and '3.0' or 'MACD Bearish (Sell)' in signals and '-3.0' or '0'}.
- **SMA**: Contributes {'Price Above SMA (Buy)' in signals and '2.4' or 'Price Below SMA (Sell)' in signals and '-2.4' or '0'}.
...
- **PSAR**: Contributes {'Price Above PSAR (Buy)' in signals and '1.0' or 'Price Below PSAR (Sell)' in signals and '-1.0' or '0'}.
...
- **Risk**: Stop-loss at 2x ATR, take-profit at 3x ATR, based on current price.
- **Sentiment**: Incorporated X-based market sentiment.
- **Price Source**: Current price sourced from yfinance.
"""
    return details
...
def main():
    ...
    if st.session_state.watchlist:
        ...
        for wt in st.session_state.watchlist:
            try:
                current_price, timestamp = fetch_current_price(wt)
                if current_price is None:
                    watchlist_data.append([wt, 'No Data', 0, 0])
                    continue
                df, info = fetch_stock_data(wt, '1mo', '1d')
                change = ((current_price - df['Close'][-2]) / df['Close'][-2] * 100) if len(df) > 1 else 0
                watchlist_data.append([wt, info.get('longName', 'N/A'), current_price, change])
            except Exception as e:
    ...
    if analyze and ticker:
        try:
            with st.spinner("Analyzing..."):
                # Fetch current price
                current_price, price_timestamp = fetch_current_price(ticker)
                if current_price is None:
                    st.error("No current price data found for the ticker.")
                    logger.error(f"No current price for ticker {ticker}")
                    return
                ...
                recommendation, signals, score, contributions = generate_recommendation(df, indicators, current_price)
                ...
                position_size = calculate_position_size(capital, risk_pct / 100, indicators['ATR'][-1] if not indicators.empty else 0, current_price)
                take_profit = current_price + 3 * indicators['ATR'][-1] if not indicators.empty else 0
                ...
                with col2:
                    ...
                    if contributions:
                        st.write("The recommendation is based on the following indicator signals and their weighted scores (using current price):")
                    ...
                    st.write(f"**Current Price**: {currency_symbol}{format_value(current_price)} (Source: yfinance, Timestamp: {price_timestamp})")
                    ...
                    st.write(f"The current price of {currency_symbol}{format_value(current_price)} was retrieved from **yfinance** at {price_timestamp}.")
    ...
    - Click 'Analyze' to view charts, recommendations, and risk management based on the current price from yfinance.
    ...
    - Recommendations incorporate ADX, PSAR, Williams %R, and X sentiment, using the current price.
    - The current price is sourced from **yfinance** with the latest available timestamp.
